...
;==============================================================
; Compact XMODEM file transfer program for CP/M 1.4 and 2.2
;
; A compact command-line driven program for transferring files
; to and from a CP/M system, using the XMODEM protocol,
; supporting both the original XMODEM checksum protocol and the
; XMODEM-CRC protocol. This program will display correctly on
; screens that are 16X64 or larger.
;
; To use XMODEM, type:
...
;
; XMODEM first looks for a file on CP/M's default drive called
; XMODEM.CFG. If found, then this file is parsed for options,
; the same as the command line. XMODEM.CFG is parsed first, so
; that options that are set on the command line will override
; those set in XMOODEM.CFG.
;
...
;     one /O option, allowing you to e.g. initialize the
;     UART and also set the baud rate.
;
;  /P Defines the direct I/O transfer port. The form of this
;     command is:
;       /P ss dd qq rr tt
;       where:
;          ss is the status port
;          dd is the data port
...
;
;  /Xn Specidies the transfer port:
...
;
;   A semicolon begins a comment. All characters from the ';'
...
;
; Based on XMODEM for CDOS (Z-80), version 1.03 by M. Eberhard
;
;==============================================================
...
;
; To Do (maybe):
;  Create new command that uses .CFG-defined calls for TXBYTE,
;    RXBYTE, so that you can use it with e.g.SOLOS, Poly-88
;  terminal mode for port testing/modem setup?
;  Support for S-100 modem?
;==============================================================
FALSE	equ	0
TRUE	equ	not FALSE
...
;used to estimate these cycle counts.
...
;------------------------------------------
;8080 Opcode, used in modified I/O routines
;------------------------------------------
...
RXBYTE:	push	h
	...
	lhld	XPORT		;h=ENHRDR
	dcr	l		;XPORT: which port?
	mov	a,h		;a=ENHRDR

	lhld	TIMRLD		;start timer
...
;--------------------------------------------
;Read character directly from I/O port.
;Timeout if no chr in e/2 seconds.
; Inner loop = 79 cycles. 0.5S / 79 uS = 6329
DIRTO	equ	6329
;--------------------------------------------
...
WAITRX:	call	RXTIMR		;(17+24)
	jc	DTIMOT		;(10)timeout?

IMODFY:	in	SIOSTA		;(10+1)status port (modified)
	ani	SIORDF		;(7)test ready (modified)
...
;inner loop:205 +CRTIME cycles
CONTO:	equ	50000/((205+CSTIME)/10)
;--------------------------------------

;wait for console character, with timeout

RXBCON:	mvi	a,CONST		;(7)get console status
	call	GOBIOS		;(116+17+CRTIME)
	ora	a		;(4)NZ means chr ready
	jnz	RXBC1		;(10) (carry is clear)

	call	RXTIMR		;(17+24)
	jnc	RXBCON		;(10)

; Carry means timeout. Otherwise get CON: character
...
;a=ENHRDR here.
;----------------------------------------------------
RXRDR:	ora	a		;Enhanced reader?
	jnz	RXERDR		;y: timer is possible

	mvi	a,READER	;BIOS call
	call	GOBIOS

MRDONE:	ora	a		;clear carry		
	pop	h
	ret			;success
...
;inner loop: 201+CRTIME cycles
RDRTO:	equ	50000/((201+CRTIME)/10)
;---------------------------------------------------------
RXERDR:	mvi	a,READER	;(7)BIOS call
	call	GOBIOS		;(116+17+BIOS time)
	jnz	MRDONE		;(10)NZ means chr ready

	call	RXTIMR		;(17+24)
	jnc	RXERDR		;(10)Z is also set if Cy set

	pop	h
	ret			;carry set for timeout
...
;  carry clear if no timeout
;  carry and Z set set if timeout
; trashes psw
; (24 cycles for inner loop)
;-----------------------------------------
...
;------------------------------
;Transmit via CP/M RDR: or PUN:
;------------------------------
	lda	XPORT		;XPORT
	ora	a		;RDR: or CON:?

	mvi	a,CONOUT	;BIOS send c to console
	jz	TXBA
...
TXBA:	call	GOBIOS		;chr in c, routine in a
	...
	pop	b		;checksum back to c
	ret

;-----------------------
;Transmit via direct I/O
;-----------------------
TXDRCT:	push	h
...
;---------------------------------
TXBTO:	lxi	d,TTOMSG	;exit message
	jmp	ABORT
...
;****************************************************
PACERR:	mvi	c,PACNAK
	db	3ah		;lda opcode skips 2 bytes
...
TTOMSG:	db	'UART Tx fail$'		;Tx not ready
...
XPORT:	db	1	;Transfer port
ENHRDR:	db	0	;nz for RDR: that returns with
...
;---------------------------------------
;Set initial character to NAK or SELCRC,
;and report if CRC mode is selected
;---------------------------------------
...
;preceeded by any reasonable number of spaces.
;-----------------------------------------------------------
...
;   top of stack = hl, pointing to next cmd byte
;   next on stack = return address to OPTLUP
; Trashes a,bc,hl
;-----------------------------------------------
	xra	a
	mov	b,a		;high byte
...
;******------------------------------------------------
;* /E * Specify Enhanced Reader (returns Z when no chr)
;******
; On Entry:
;    a=0
;   de=next command line chr
...
; help screen so that it fits on a 16x64 screen
;************************************************
HLPEXT:	call	CILPRT		;print this message

;    1234567890123456789012345678901234567890123456789012345678901234
 db '=========================',CR,LF
 db 'XMODEM 2.3 By M. Eberhard',CR,LF
 db '=========================',CR,LF
 db 'Usage:',CR,LF
 db ' XMODEM filename options',CR,LF
 db '^C aborts',CR,LF
 ...
 db ' /E if RDR returns with Z set when not ready',CR,LF
 db ' /M console message',CR,LF
 db ' /O pp h1 ... hn outputs bytes h1-hn to port pp',CR,LF
 db '--More-','-'+80h
 ...
 db '   qq = 00 if ready bits are true low, 01 if true high'
 db CR,LF
 db '   rr = Rx ready bit mask',CR,LF
 db '   tt = Tx ready bit mask',CR,LF
 db ' /Q for Quiet; else + for good block, - for retry'
 db CR,LF
 db ' /X0 use CON',CR,LF
 db ' /X1 use RDR/PUN (default)',CR,LF
 db ' /X2 use direct I/O, default 88-SIO',CR,LF
 db ' /Zm for m MHz CPU. 0<m<10, default m=2',CR,LF
 db 'CON and RDR must not strip parity.',CR,LF
 db 'Values for /O and /P are 2-digit hex.'
 db '$'
...
MODIO:	mov	m,e		;install status port
	...
	mov	a,c
	ora	a
	jz	MODIO1		;need a jz opcode?
	mvi	m,JZOP		;y: install jz
MODIO1:
